...
object GoogleVsApple:
  ...
  private lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(tweet => google.exists(elem => tweet.text.contains(elem)))

  private lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(tweet => apple.exists(elem => tweet.text.contains(elem)))
