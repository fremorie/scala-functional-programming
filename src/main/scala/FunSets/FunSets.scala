...
trait FunSets extends FunSetsInterface:
   ...
   */
  def union(s: FunSet, t: FunSet): FunSet = (elem: Int) =>
   ...
   */
  def diff(s: FunSet, t: FunSet): FunSet = (elem: Int) =>
    contains(s, elem) && !contains(t, elem)

  /**
   * Returns the subset of `s` for which `p` holds.
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = (elem: Int) =>
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = elem => exists(s, y => f(y) == elem)
